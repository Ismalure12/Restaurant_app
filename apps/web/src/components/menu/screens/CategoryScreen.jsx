...
export default function CategoryScreen() {
  const {
    ...
    ordered, goBack, openCart, openCategory, openDetail, quickAdd,
          ...
          <>
            ...
            <div className="cat-tabs" ref={catTabsCatRef}>
              <div className="scroll-row" ref={catnavCatRef}>
                ...
                {ordered.map((cc) => (
                  <button key={cc.slug} className={`cat ${cc.slug === currentCatSlug ? 'active' : ''}`} data-cat={cc.slug} onClick={() => openCategory(cc.slug, 'slide')}>
                    <span className="pill" />{cc.name}<span className="count">{cc.items.length}</span>
                  </button>
                ))}
              </div>
            </div>
