 ...
 */
export async function getAvgCostMap(prisma: Db, itemIds?: number[]): Promise<Map<number, number | null>> {
  const where: Prisma.StockMovementWhereInput = { type: 'purchase' };
  if (itemIds) {
...
export async function getAvgCost(prisma: Db, inventoryItemId: number) {
...
export function effectiveCost(avgCost: number | null | undefined, costPerUnit: unknown) {
