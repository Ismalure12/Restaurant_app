...
- **Image uploads:** `POST /api/upload` → sharp webp → S3 (`lib/storage/s3.ts`, public URL `https://<bucket>.s3.<region>.amazonaws.com/menu/…` or `S3_PUBLIC_URL`). Older rows still hold Vercel Blob URLs (kept in `next.config.mjs` `remotePatterns`).
