...
// Resolved like any import: npm workspaces hoist tailwindcss to the repo root.
const TAILWIND_CLI = createRequire(import.meta.url).resolve('tailwindcss/lib/cli.js');
execFileSync(process.execPath, [TAILWIND_CLI, '-c', path.join(ROOT, 'tailwind.config.js'), '-i', path.join(ROOT, 'src/styles/globals.css'), '-o', OUT], { cwd: ROOT, stdio: 'pipe' });
