...
function Crumb({ customer, label }) {
  const root = { href: '/admin/dashboard/customers', label: 'Customers' };
  return (
    ...
    <div className="wrap-narrow">
      ...
      <div className="detail-h">
        <div>
          ...
          <div className="od-tags">
            ...
            {invoice.order?.code && <Link className="pill pill-ghost" href={`/admin/dashboard/orders/${invoice.order.id}`}>Order {invoice.order.code}</Link>}
      ...
      <div className="grid2 reveal" style={{ animationDelay: '.1s' }}>
            ...
            {invoice.payments.length === 0 ? (
              ...
              <div className="mv-item" key={p.id}>
                ...
                <span className="mv-main">
                  <span style={{ fontWeight: 600, color: 'var(--ink)' }}>{p.account || METHOD_LABEL[p.method] || p.method}</span>
        ...
        <div className="card card-pad">
          ...
          {canPay ? (
            <form noValidate onSubmit={handlePay}>
              <Field label={`Amount · balance ${money(balance)}`} required {...payForm.fieldProps('amount')}>
                {(a) => (
                  <div style={{ display: 'flex', gap: 8 }}>
                    <input {...a} className={`input${a['aria-invalid'] ? ' input-err' : ''}`} type="number" min="0" step="0.01" inputMode="decimal" value={payAmount} onChange={(e) => setPayAmount(e.target.value)} placeholder="0.00" />
                    <button type="button" className="btn btn-soft" onClick={() => setPayAmount(balance.toFixed(2))}>Full</button>
                  </div>
                )}
              </Field>
              <Field label="Method" required {...payForm.fieldProps('accountChosen')}>
                {() => (
                  <div className="chips">
                    {payChips.map((o) => (
                      <button key={o.key} type="button" className={`chip2${payMethod === o.key ? ' on' : ''}`} onClick={() => setPayMethod(o.key)} title={o.hint || undefined}>{o.label}</button>
                    ))}
                  </div>
                )}
              </Field>
              ...
              <button className="btn btn-primary btn-block" type="submit" style={{ marginTop: 14 }} disabled={payMutation.isPending || !payForm.valid}>{payMutation.isPending ? 'Recording…' : 'Record payment'}</button>
            </form>
          ) : (
